...
def runClicked():
    ...
    for file in window.filepath:
        ...
        runString = pycmd.get() + " ocr2.py -i \"" + file + "\" -p " + \
                preprocCombo.get() + " -l " + lang.get() + " -s " + \
                lbCombo.get() + " -b " + pgCombo.get() + " -z "+ qCombo.get()
        runStrings.append(runString)

        if (file == "Choose..." or file == ""):
            messagebox.showerror("Error", 
                                 "Please choose file(s) on which to run the script.")
            return

    result = messagebox.askyesno("Ok to execute these " + str(len(runStrings)) \
                                 + " scripts?", runStrings[0])    
    if (result != True):
        print("Canceled.")
        return
    ...
    for i in range (0, len(runStrings)):
        ...
        window.title("David's Tesseract-OCR frontend : Script "\
                     + str(i+1) + " of " + str(len(runStrings)) +\
                     " is running.")
        print(">>" + runString)
        os.system(runString)
...
def fileClicked():
    window.filepath = \
            filedialog.askopenfilenames(initialdir ="images",
                                        title = "Select image file or folder of images",
                                        filetypes = (("image files","*.jpg *.png *.tif *.tiff *.bmp *.pdf *.gif"),("all files","*.*")))
    print(window.filepath)

    file_text.set(str(str(len(window.filepath)) + ": " + \
                      window.filepath[0][window.filepath[0].rfind("/")+1:])[:15])
